...
function initMonth() {
    ...
    // Determine mandatory CC minimum payment
    let ccMin = 0;
    if(state.debt > 0) {
...
function renderCategories() {
    if (!els.fixedList || !els.varList) return;
...
function runMonth() {
    ...
    if (els.evChoices) els.evChoices.innerHTML = '';
...
function triggerForcedEvent(title, desc, cost, isIncome=false) {
    ...
    if(cost < 0) {
        ...
        if(result.cc > 0) {
            els.evStatus.innerText = `EF drained. ${format(result.cc)} went to CREDIT DEBT!`;
            playSound('bankrupt');
        } else {
...
function prepareEOM() {
    ...
    if(state.debt > 0) {
        ...
        let interest = Math.floor(state.debt * 0.02);
